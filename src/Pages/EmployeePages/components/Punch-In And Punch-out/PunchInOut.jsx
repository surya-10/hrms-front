    ...
    try {
      ...
      if (response.data && response.data.data && Array.isArray(response.data.data)) {
        const approvedLeave = response.data.data.some(leave => {
          // Check if leave has the required properties
          // console.log(leave)
          ...
          return leave.leave_date.some(date => {
            ...
            if (isMatchingDate && leave.is_half_day_leave && leave.status_name === "Approved") {
              ...
              if (isFirstHalf && currentHour < 14) {
                setLeaveMessage("You have an approved leave for the first half of the day");
                return true;
              } else if (!isFirstHalf && currentHour >= 14) {
      ...
      // Don't block punch-in if there's an error checking leaves
