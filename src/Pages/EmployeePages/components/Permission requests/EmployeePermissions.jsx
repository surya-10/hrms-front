...
export default function PermissionDropdown() {
    ...
    return (
        <div className="mx-auto bg-white shadow-lg rounded-xl p-6 mt-6 mb-6">
            <div className="flex justify-between items-center gap-4">

                <h3 className="text-lg font-semibold text-[#2D3748]">Permissions</h3>

                <div className='flex items-center gap-4'>
                    <h2 className="text-lg font-[500] text-gray-500">Select Month</h2>
                    <div>
                        <select
                            className="p-3 border rounded-lg text-gray-700 focus:ring-2 focus:ring-[#2C7A7B] outline-none bg-white"
                            value={selectedMonth}
                                ...
                                <tbody>
                                    {filteredPermissions.map((permission, index) => (
                                        <tr key={index} className="hover:bg-[#E6FFFA]">
                                            <td className="border border-gray-300 px-4 py-2">{new Date(permission.date).toDateString()}</td>
                                            <td className="border border-gray-300 px-4 py-2">{permission.startTime}</td>
                                            <td className="border border-gray-300 px-4 py-2">{permission.endTime}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
