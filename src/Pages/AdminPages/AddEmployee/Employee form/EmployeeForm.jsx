...
import { Camera, User, Mail, Lock, Phone, Users2, ClipboardCheck, Copy, CopyCheck } from 'lucide-react';
...
const EmployeeForm = () => {
  ...
  const handleCopy = () => {
    ...
    setCopied(true);
    setTimeout(() => setCopied(false), 1500); // Reset icon after 1.5 sec
  };
    ...
    const fetchEmployee = async () => {
      try {
        ...
        const managersData = response.data.data
          .filter(employee => employee.role.role_value === 'manager' || employee.role.role_value ==="admin")
          .map(employee => ({
  ...
  const handleSubmit = async (e) => {
    ...
    e.preventDefault();
    setLoading(true)
    localStorage.setItem("createdUserEmail", formData.email);
    try {
      ...
      if (response.data.message == "Employee already created") {
        toast.error("Employee already created")
        return
      }
      else {
            ...
            >
              ...
              >
                ...
                <div className="flex gap-2">
                  ...
                  <p className="text-gray-500 hover:text-indigo-500 text-center mt-1 cursor-pointer" title='copy' onClick={handleCopy}>
                    {!copied ? <p className='text-indigo-400 bg-indigo-100 p-2 rounded-lg'><Copy /></p> : <p className='text-indigo-800 bg-indigo-200 p-2 rounded-lg'> <CopyCheck /></p>}
