...
const LeaveDetailsForm = ({ data, onClose, updateLeaveStatus }) => {
                        ...
                        <div className="space-y-3">
                            ...
                            {data.leave_date?.length > 0 && (
                                <div>
                                    ...
                                    {data.leave_date.map((datee, index) => (
                                        <div key={index} className="bg-gray-50 p-2 rounded-lg text-sm">
                                            <span className="font-medium text-sm">
                                                {!data.is_permission && data.is_half_day_leave ? 
                                                <div>
                                                     <span>{formatDate(datee.date)}•{datee.start_date=="9 AM" ? "First Half":"Second Half"}</span>
                                                </div>
                                                :
                                                    <div>
                                                        {data.is_permission ? <span>{datee.start_date}</span>
                                                            :
                                                            <span>{new Date(datee.start_date).toLocaleDateString()}</span>
                                                        }
                                                        {" to "}

                                                        {data.is_permission ? <span>{datee.end_date}</span>
                                                            :
                                                            <span> {new Date(datee.end_date).toLocaleDateString()}</span>
                                                        }
                                                    </div>
                                                }
                                            </span>
